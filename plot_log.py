...
def create_field_index():
    ...
    test_key = 'Test'
    field_index = \
        {train_key: {'Iters': 0, 'Seconds': 1, train_key + ' loss': 2,
...
def plot_chart2(chart_type_lst, path_to_png, path_to_log_list):
    """
    DIfferent with the original version, this plot_chart2 can plot multi
    chart_type on a single figure, such as train and test
    """
    plt.ion()
    for path_to_log in path_to_log_list:
        # Get the path of the png, which will used to store the parsed log
        path_of_png = path_tools.get_path(path_to_png)
        os.system('%s %s %s' %
                  (get_log_parsing_script(), path_to_log, path_of_png))
        for chart_type in chart_type_lst:
            data_file = get_data_file(chart_type, path_to_log)
            data_file = path_tools.get_full_path(path_of_png, data_file)
            x_axis_field, y_axis_field = get_field_descriptions(chart_type)
            x, y = get_field_indices(x_axis_field, y_axis_field, chart_type)
            data = load_data(data_file, x, y)
            # TODO: more systematic color cycle for lines
...
Usage:
    ./plot_training_log.py chart_type[0-%s] \
/where/to/save.png /path/to/first.log ...
Notes:
...
def split_char_type(chart_type_str):
    chart_type_lst = []
    for ch in chart_type_str:
        chart_type_lst.append(int(ch))
    return chart_type_lst
...
def main(argv):
    if len(sys.argv) < 4:
        ...
        chart_type_lst = split_char_type(sys.argv[1])
        for chart_type in chart_type_lst:
        ...
        try:
            plot_chart2(chart_type_lst, path_to_png, path_to_logs)
        except:
            pass
